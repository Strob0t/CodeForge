...
The agent execution loop involves receiving a task, planning, requesting tool calls, executing tools, evaluating results, checking policies, and delivering results. This loop has both state-heavy components (session tracking, policy evaluation, checkpoints) and compute-heavy components (LLM calls, tool execution, code analysis).
...
| Tool execution results | Python Worker to Go Core | NATS to PostgreSQL |
...
- Policy enforcement is centralized: All policy decisions happen in Go, so Python workers never bypass safety rules
