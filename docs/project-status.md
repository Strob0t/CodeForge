...
- [x] (2026-03-07) AG-UI Protocol Events — CopilotKit Agent-User Interaction (8 event types emitted via WebSocket: run_started, run_finished, text_message, tool_call, tool_result, state_delta, step_started, step_finished; Go Core emits in runtime.go + conversation.go; frontend ChatPanel + websocket.ts consume events for streaming display; 20+ files across Go, Python, and TypeScript)
