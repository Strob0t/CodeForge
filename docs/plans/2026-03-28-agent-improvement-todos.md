...
**Fix:**
- [ ] When `capability_level == "pure_completion"`, set `tools=None` in the `chat_completion_stream()` call
- [ ] Instead, inject tool definitions into the system prompt via `tool_guide.py` (already exists)
- [ ] Add a text-extraction parser that detects tool calls from free-text responses (regex for JSON blocks like `{"tool": "write_file", "args": {...}}`)
- [ ] Test with qwen3-30b: send agentic message, verify tool calls are extracted from text
...
**Fix:**
- [ ] Query `model_info.max_input_tokens` from LiteLLM API (`GET /model/info`) at run start
- [ ] Set context limit to `min(actual_model_window * 0.85, tier_default)` (0.85 leaves room for output)
- [ ] Scale `DEFAULT_TOOL_OUTPUT_MAX_CHARS` proportionally: `min(10000, context_limit * 4 // 12)`
- [ ] Log actual context budget in run metadata for debugging
...
**Fix:**
- [ ] After each LLM response, check: if `finish_reason == "stop"` AND `tool_calls == []` AND `iteration == 0` AND `len(content) > 100`
- [ ] Re-prompt with: "You have tools available. Use them to complete the task. Call list_directory or read_file to start."
- [ ] Allow max 2 re-prompts before falling back to text-extraction parser (TODO-1)
- [ ] Add metric: `agent.no_tool_call_reprompts`
...
**Fix:**
- [ ] Add to base system prompt (not just tool guide): "MANDATORY: Before editing any file, read it first with read_file. Before modifying a codebase, explore with list_directory and glob_files."
- [ ] In `ToolExecutor.execute()`: when `edit_file` is called for a path not yet `read_file`'d in this session, inject warning into tool result: "WARNING: Editing {path} without reading it first."
- [ ] Track `files_read` set in `_LoopState` to enable this check
...
**Fix:**
- [ ] Track `writes_since_last_verify` counter in `_LoopState`
- [ ] Increment on `write_file`/`edit_file`, reset on `bash` calls containing `test`/`pytest`/`compile`/`tsc`
- [ ] When counter >= 3, inject nudge into next LLM turn: "You've written 3 files without verification. Run tests or syntax checks now."
- [ ] Add to system prompt: "After writing each file, verify it. Never write multiple files without testing between them."
...
**Fix:**
- [ ] Replace generic `STALL_ESCAPE_PROMPT` with context-specific prompts based on stall type:
...
- [ ] Include summary of recent tool results in escape prompt (what the agent has accomplished)
- [ ] After 2 escape attempts, try MagenticOne-style re-planning: summarize accomplishments, ask for new plan, reset stall counter, resume
...
**Fix:**
- [ ] After each tool call, append brief state summary to tool result (~50 tokens):
  ```
  [State: files_modified=[main.py], files_read=[utils.py, config.py], tests_run=0, iteration=3/50]
  ```
- [ ] Track in `_LoopState`: `files_modified`, `files_read`, `tests_run`, `errors_seen`
- [ ] Only inject for models with < 64K context (unnecessary overhead for large-context models)
...
**Fix:**
- [ ] Add optional request logging: when `APP_ENV=development`, log the full request body (messages, tools, tool_choice, model) to a file before each `chat_completion_stream()` call
- [ ] Log path: `/tmp/codeforge-llm-requests/{run_id}/{iteration}.json`
- [ ] Include response summary (finish_reason, tool_calls count, content length)
- [ ] Add env var `CODEFORGE_LOG_LLM_REQUESTS=true` to enable in non-dev environments
...
**Fix:**
- [ ] At run completion, compute and publish trajectory metrics:
...
- [ ] Publish as part of `runs.complete` NATS payload
- [ ] Display in run detail view in frontend

### TODO-10: Multi-run evaluation with pass@k
**Problem:** Single runs are noisy. Need statistical methodology.
**Fix:**
- [ ] Add `repeat_count` parameter to conversation dispatch (run same prompt N times)
- [ ] Compute pass@k: `1 - C(n-c, k) / C(n, k)` where n=total runs, c=passing runs, k=attempts
- [ ] For S1-S4, run 5x each and report pass@5 instead of single-run pass/fail
- [ ] Store results in `benchmark_runs` table (already exists)
...
**Fix:**
- [ ] After complexity analysis, check if selected model can handle the tier
- [ ] If `complexity >= COMPLEX` and `model.max_tokens <= 32768`, inject decomposition prompt: "This is a complex task. Break it into smaller subtasks and tackle each one sequentially."
- [ ] Reduce `max_iterations` for SIMPLE tasks on local models (15 instead of 50)
- [ ] Set `max_tokens` output from `_estimate_output_tokens()` (already computed but unused)
...
**Fix:**
- [ ] Create 3 prompt tiers: full (120K+), compressed (32-64K), minimal (< 32K)
- [ ] Compressed: remove examples from tool guide, shorten descriptions
- [ ] Minimal: only essential rules, no examples, no context entries
- [ ] Select tier based on actual model context window (from TODO-2)
...
**Fix:**
- [ ] Before each LLM call, check `estimated_tokens / context_limit`
- [ ] If ratio > 0.7, trigger `ConversationSummarizer` immediately
- [ ] If ratio > 0.9, aggressive truncation: remove all tool results except last 5

### TODO-14: Automated S1-S4 regression test
**Problem:** Manual test execution — should be automated.
**Fix:**
- [ ] Create `scripts/run-agent-eval.sh` that:
...
- [ ] Add to CI as a nightly job (not on every PR — too slow for local models)
- [ ] Support `--model` flag to test with different models
