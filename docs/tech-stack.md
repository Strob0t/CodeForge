...
- LiteLLM Proxy (Docker Sidecar, Port 4000) — central LLM gateway (Image: `ghcr.io/berriai/litellm:main-stable`, 127+ providers, 6 routing strategies, budget management; Config: `litellm/config.yaml` generated by Go Core; Dependencies: PostgreSQL shared instance, Redis optional for multi-instance only)
...
- Custom WebSocket wrapper (~100 LOC) — auto-reconnect + heartbeat (no external WebSocket library)
