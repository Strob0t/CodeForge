...
MCP is an open standard (November 2024) that provides a universal interface for AI models to access external tools, data sources, and services. It uses JSON-RPC 2.0 over stdio (local) or HTTP+SSE (remote). Anthropic donated it to the Linux Foundation's AAIF in December 2025.
...
Architecture uses Host / Client / Server roles:
...
A2A is an open protocol for communication between opaque AI agents from different providers and frameworks. Google announced it April 2025 at Google Cloud Next. It uses JSON-RPC 2.0 over HTTP(S) and is now governed by the Linux Foundation (v0.3, July 2025).
...
- CodeForge agents as A2A servers: External agents can discover and delegate tasks to CodeForge's specialized agents (e.g., an enterprise workflow agent asks CodeForge to "review this PR" or "implement this feature"). CodeForge publishes an Agent Card describing its capabilities.
- CodeForge consuming external A2A agents: CodeForge's orchestrator can discover and delegate to external specialized agents (e.g., a security scanning agent, a documentation agent) that expose A2A endpoints.

Implementation complexity is low-medium. A2A is HTTP-based (no special transport). Publishing an Agent Card is trivial. The task lifecycle model maps well to CodeForge's existing task workflow (Plan/Approve/Execute/Review/Deliver). Python SDK available.
...
ACP was a REST-based agent-to-agent protocol launched March 2025 by IBM Research for the BeeAI platform. It distinguished itself by requiring no SDK (plain HTTP), offline discovery, and OTLP-instrumented observability.
...
AG-UI is an open protocol (May 2025) for bi-directional communication between AI agents and user-facing applications. It streams JSON events over HTTP: messages, tool calls, state patches, and lifecycle signals. The project has 9,000+ GitHub stars and 120,000+ weekly installs.
...
ANP is a decentralized protocol for agent discovery, identity, and encrypted communication across organizations. It uses W3C Decentralized Identifiers (DIDs), JSON-LD metadata, and end-to-end encryption. ANP aims to be the "HTTP of the Agentic Web."

**Three-layer architecture:**

- Identity and encrypted communication (DIDs, cryptographic signatures)
- Meta-protocol negotiation (agents negotiate how to communicate)
- Application protocol (structured capability discovery via JSON-LD)

ANP solves the problem of agents from different organizations discovering and trusting each other across the open internet. This is relevant for enterprise scenarios where CodeForge instances at different companies need to collaborate, or for an agent marketplace.
...
AGENTS.md is a simple, open Markdown format for providing AI coding agents with project-specific instructions. Think of it as a README for agents. Over 60,000 open-source projects adopted it since August 2025. It is now governed by the AAIF under the Linux Foundation.

An `AGENTS.md` file at the repo root (or in subdirectories) contains build steps, coding conventions, test commands, and other context that coding agents need. Agents automatically read the nearest file in the directory tree.

**Relevance for CodeForge:**

CodeForge should both read and write AGENTS.md:

- Read: When CodeForge clones a repo, the Context Layer should automatically parse any `AGENTS.md` files and inject their content into agent prompts. This provides project-specific instructions without user configuration.
- Write: CodeForge could generate or update `AGENTS.md` files based on learned project conventions, similar to how `.codeforge/microagents/` work but in a standardized format other tools understand.
- Interoperability: Since AGENTS.md is supported by Amp, Codex, Cursor, Devin, Gemini CLI, GitHub Copilot, Jules, and VS Code, CodeForge agents benefit from instructions already written for other tools.

Implementation complexity is trivial. It is just Markdown parsing. No SDK, no protocol, no transport layer.
...
OpenAI's standard for structured tool/function calling from LLMs uses JSON Schema-based function definitions with strict mode for guaranteed schema adherence. It is the de facto standard that LiteLLM normalizes all providers to.

Already fully covered by the **LiteLLM** integration. LiteLLM handles the conversion of tool calling across 127+ providers. CodeForge's YAML tool bundles are converted to OpenAI function calling format. No additional work needed.
...
LSP is a protocol (by Microsoft, 2016) that standardizes how IDEs communicate with language-specific analysis servers. It provides go-to-definition, find references, diagnostics, completions, hover documentation, and symbol navigation.
...
Run language servers inside agent sandbox containers or as sidecar services. Expose LSP capabilities as MCP tools (using existing LSP-MCP bridge servers). This avoids building a custom LSP client in Go -- instead, the Python workers connect to LSP servers via the standard protocol and expose results as tool calls.

LSP servers are per-language (TypeScript needs `typescript-language-server`, Python needs `pyright` or `pylsp`, Go needs `gopls`, etc.). CodeForge must auto-detect the repo's primary languages and start the right servers. This adds container complexity but is worth the precision improvement.
...
DAP is a protocol (by Microsoft) that abstracts debugger communication, similar to how LSP abstracts language analysis. It supports breakpoints, stepping, variable inspection, and expression evaluation across languages.
...
Use existing MCP-DAP bridge servers rather than building a custom DAP client. The `debugger` agent mode (already defined in architecture) would gain access to DAP tools via MCP.
...
Tree-sitter is a parser generator and incremental parsing library (not a protocol). Written in C, dependency-free, with bindings for Rust, Python, Go, JavaScript, and more. It parses source code into concrete syntax trees (CSTs) for structural analysis.

Already planned for the tree-sitter repo map (from Aider's pattern). Tree-sitter is a library, not a protocol -- it runs locally within the Python workers. Key ecosystem developments:
...
SARIF is an OASIS standard (JSON-based) for representing the output of static analysis tools. Version 2.1.0 provides a rich schema for findings including source location, severity, remediation guidance, CWE/CVE identifiers.
...
- Input: CodeForge's `security-auditor` and `reviewer` modes can consume SARIF output from existing static analysis tools (bandit, semgrep, ESLint, SonarQube). This gives agents structured, machine-readable analysis results to reason about instead of parsing tool-specific text output.
- Output: CodeForge's agent review results could be emitted as SARIF, making them consumable by GitHub Code Scanning, VS Code, and other tools that understand SARIF. This provides interoperability with existing security workflows.
...
OpenTelemetry is the CNCF standard for distributed tracing, metrics, and logs. The GenAI Semantic Conventions SIG (started April 2024) defines standard attribute names for LLM calls, agent steps, sessions, token counts, costs, and quality metrics.
...
- Agent run tracing: Each agent workflow (Plan/Execute/Review/Deliver) maps naturally to OTel spans. Sub-spans for individual LLM calls, tool executions, and agent steps provide the detailed observability needed for debugging failed runs, optimizing costs, and comparing agents.
- Cross-service correlation: CodeForge has three services (Go Core, Python Workers, LiteLLM). OTel trace context propagation through NATS messages connects the full request lifecycle from frontend click to LLM response.
- Vendor-agnostic export: OTel data can be sent to any backend (Jaeger, Grafana Tempo, Datadog, Langfuse) without code changes. This aligns with CodeForge's provider-agnostic philosophy.
...
The Event Bus (from CrewAI pattern) emits events for the frontend. OTel traces are the backend-side equivalent for operational observability. They complement each other -- events are for users, traces are for operators.
...
Prometheus is the CNCF standard for time-series metrics collection via a pull-based scrape model. LiteLLM already exposes comprehensive Prometheus metrics.
...
- Expose LiteLLM's metrics: Configure LiteLLM's Prometheus metrics and expose them to the host for scraping
- Add CodeForge-specific metrics: Agent task counts, queue depth, execution times, sandbox container lifecycle
- Build Grafana dashboards: Cost per model over time, P50/P90/P99 latencies, error rates, agent success rates
...
These are three related standards:
...
- GitHub/GitLab authentication: Clone private repos, create PRs, access issues. Both platforms use OAuth 2.0 with OIDC support.
- MCP remote server auth: The MCP spec (2025-11-25) mandates OAuth 2.1 for remote server authorization. CodeForge as an MCP host must implement the OAuth flow.
- PM platform auth: Plane.so, OpenProject, and other PM tools use OAuth 2.0 for API access.
- User login: CodeForge's own authentication can use OIDC to federate with GitHub, GitLab, Google, etc.
- CI/CD OIDC tokens: GitHub Actions and GitLab CI emit OIDC tokens for keyless authentication.
...
SCIM is an open standard (RFC 7643/7644) for automated user provisioning and deprovisioning across systems. It is REST-based with JSON payloads.
...
SCIM is relevant only for enterprise multi-tenancy: automated onboarding/offboarding of users from corporate identity providers (Okta, Azure AD, etc.). Not needed for single-instance or small-team deployments.
...
OpenAPI is the standard for describing REST APIs. Version 3.1 (February 2021) aligned with JSON Schema 2020-12. Version 3.2.0 (September 2025) adds structured tag navigation, streaming media types, and new OAuth flows.

**Relevance for CodeForge:**

- API documentation: CodeForge's REST API should be documented with OpenAPI. This enables auto-generated API docs, client SDKs, and validation.
- MCP tool discovery: MCP tools can be generated from OpenAPI specs. This means any service with an OpenAPI spec can become an MCP server automatically (Apigee already does this).
- A2A Agent Card: The A2A Agent Card references security schemes from the OpenAPI specification.
...
JSON-RPC 2.0 is a lightweight remote procedure call protocol encoded in JSON. It is stateless and transport-agnostic.

JSON-RPC is the transport for both MCP and A2A. CodeForge does not need to implement JSON-RPC independently -- the MCP and A2A SDKs handle it. However, understanding JSON-RPC is important for debugging MCP connections and implementing custom MCP servers.
...
CodeForge's Python workers need a robust MCP client implementation as their primary extension mechanism. The `McpWorkbench` pattern (already planned) is the right abstraction -- it wraps any MCP server as a tool container that can be dynamically loaded based on task context.
