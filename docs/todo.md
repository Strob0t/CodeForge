...
- [x] (2026-03-07) P0: Fix 10 missing tenant isolation queries — `store_a2a.go` (8 queries: Get/Update/Delete A2ATask, Get/Update/Delete RemoteAgent, List tasks/agents) + `store_conversation.go` (2 queries: Get/Delete Conversation); parameterize LIMIT; add `tenantctx` shared package; propagate tenant_id through NATS A2A payloads; 6 new integration tests
...
#### 18G: Fix Knowledge Base System (3 Critical Bugs + Cleanup)

> The Knowledge Base feature is completely non-functional due to 3 critical bugs in the indexing
> pipeline. Additionally, the 8 built-in KBs reference content directories that don't exist and
> will be removed — only user-created custom KBs are supported.
>
> Root cause analysis:
> - Bug 1: `RetrievalService.RequestIndex()` calls `GetProject()` with a `"kb:..."` ID, but KBs
>   are NOT in the `projects` table → always fails with ErrNotFound.
> - Bug 2: `RetrievalService.HandleIndexResult()` updates in-memory state but NEVER calls
>   `UpdateKnowledgeBaseStatus()` → KB status stays "pending" forever in the DB, even on success.
> - Bug 3: The 8 built-in KBs reference `data/knowledge/{name}` directories that don't exist.
>
> Decision: Remove built-in catalog entirely (Option A). Only custom KBs with user-provided
> content paths are supported. Fix the indexing pipeline to handle KB projects natively.
...
> Problem: `RequestIndex()` at `internal/service/retrieval.go:137` calls `s.store.GetProject(ctx, projectID)`
> with `"kb:<uuid>"` — this always fails because KBs are in a separate table.
...
> Problem: `HandleIndexResult()` at `internal/service/retrieval.go:178` updates in-memory state
...
- [x] (2026-02-25) Add `benchmark.run.request` subject — Go Core publishes, Python Worker subscribes
- [x] (2026-02-25) Add `benchmark.run.result` subject — Python Worker publishes results back
- [x] (2026-02-25) Message models: `BenchmarkRunRequest`, `BenchmarkTaskResult`, `BenchmarkRunResult` in `models.py`
- [x] (2026-02-25) Subscribe in `consumer.py` — `_handle_benchmark_run()` method, gated by `APP_ENV=development`
...
- [x] (2026-02-25) Add dev-mode check middleware: `DevModeOnly` in `internal/middleware/devmode.go` — reads `APP_ENV` env var, returns 403 if not `development`
- [x] (2026-02-25) `POST /api/v1/benchmarks/runs` — create a benchmark run (stores in DB)
- [x] (2026-02-25) `GET /api/v1/benchmarks/runs` — list all benchmark runs with summary scores
- [x] (2026-02-25) `GET /api/v1/benchmarks/runs/{id}` — get detailed run info
- [x] (2026-02-25) `DELETE /api/v1/benchmarks/runs/{id}` — delete a benchmark run and its results
- [x] (2026-02-25) `GET /api/v1/benchmarks/datasets` — list available benchmark datasets (reads `configs/benchmarks/*.yaml`)
- [x] (2026-02-25) `POST /api/v1/benchmarks/compare` — compare two runs side-by-side
- [x] (2026-02-25) `GET /api/v1/benchmarks/runs/{id}/results` — list results for a run
...
- [x] (2026-02-25) Domain types in `internal/domain/benchmark/benchmark.go` (Run, Result, DatasetInfo, CreateRunRequest, CompareRequest, CompareResult)
- [x] (2026-02-25) Store interface methods in `internal/port/database/store.go` (7 benchmark methods)
- [x] (2026-02-25) Postgres implementation in `internal/adapter/postgres/store_benchmark.go`
- [x] (2026-02-25) BenchmarkService in `internal/service/benchmark.go` with dataset scanning

**Config:**

- [x] (2026-02-25) Add `benchmark` section to config:
  ```yaml
  benchmark:
    enabled: false
    datasets_dir: "configs/benchmarks"
    timeout_seconds: 300
    dashboard_port: 3100
  ```
...
- [x] (2026-02-25) Add i18n keys for all benchmark UI strings (27 keys in en.ts + de.ts)
...
> A2A (Agent-to-Agent Protocol, Linux Foundation, v0.3.0) enables secure, interoperable communication between AI agents. This phase upgrades the existing discovery-only stub to a full implementation using the official `a2a-go` SDK. CodeForge acts as both A2A **server** (exposing agents to external callers) and A2A **client** (delegating to remote A2A agents).

- [x] (2026-03-02) Phase 27A: SDK Installation + Interface Verification (`go get github.com/a2aproject/a2a-go@v0.3.7`, verified AgentExecutor/TaskStore/AgentCardProducer interfaces)
- [x] (2026-03-02) Phase 27B: Domain Types (`internal/domain/a2a/task.go`, `remote_agent.go` — A2ATask, TaskState 8 values, RemoteAgent, Validate, 14 tests)
- [x] (2026-03-02) Phase 27C: Database Migration + Store Methods (`054_a2a_protocol.sql` — 3 tables, 15 store methods in `store_a2a.go`, mock stubs updated in 4 test files)
- [x] (2026-03-02) Phase 27D: SDK TaskStore Adapter (`internal/adapter/a2a/taskstore.go` — Save/Get/List, domain↔SDK conversion, optimistic locking)
- [x] (2026-03-02) Phase 27E: AgentExecutor Implementation (`internal/adapter/a2a/executor.go` — Execute/Cancel, NATS publish, WS broadcast, trust annotation)
- [x] (2026-03-02) Phase 27F: Dynamic AgentCard Builder (`internal/adapter/a2a/agentcard.go` — modes→skills mapping, security schemes, provider info)
- [x] (2026-03-02) Phase 27G: A2A Authentication Middleware (`internal/middleware/a2a_auth.go` — Bearer token validation, trust levels, 7 tests)
...
### Phase 28: R2E-Gym / EntroPO Integration — Hybrid Verification, Test-Time Scaling & Diversity

> **Source:** [R2E-Gym](https://github.com/R2E-Gym/R2E-Gym) (COLM 2025, UC Berkeley) + [EntroPO](https://arxiv.org/abs/2509.12434) (arXiv 2509.12434, Northwestern)
>
> **Why:** R2E-Gym demonstrates that hybrid verification (execution filter + LLM ranking) reaches 51% on SWE-bench
> versus ~42% for either approach alone. EntroPO shows that entropy-regularized preference optimization prevents
> diversity collapse during test-time scaling — a 30B model achieves 59.8% SWE-bench Verified with 16 rollouts
...
> **Insight:** R2E-Gym's two-stage approach — execution-based filtering first (binary pass/fail),
> then execution-free ranking (LLM continuous scoring) of survivors only — eliminates wasted LLM tokens
> on broken outputs and yields complementary signal (51% vs 42-43% for either stage alone).
...
- [x] (2026-03-02) `workers/tests/test_hybrid_pipeline.py` — Tests:
  ...
  - Empty evaluator lists (filter-only, rank-only modes)
  - Filter threshold edge cases (0.0, 1.0, exactly at threshold)
  - Single result (degenerate case, no batch logic)
...
**Reuses:** `EvaluationPipeline` (`workers/codeforge/evaluation/pipeline.py`), `Evaluator` protocol (`evaluators/base.py`), `EvalDimension`/`EvalScore` (`providers/base.py`), `FunctionalTestEvaluator`, `LLMJudgeEvaluator`

---

#### 28B: Trajectory Verifier Evaluator ✅ (2026-03-02)

> **Insight:** R2E-Gym uses a dedicated verifier model that sees the FULL trajectory (all tool calls,
> intermediate steps, file diffs, test output) — not just input/output like `LLMJudgeEvaluator`.
> The verifier can detect approach quality, error recovery patterns, and completeness that output-only
...
- [x] (2026-03-02) `workers/codeforge/evaluation/evaluators/trajectory_verifier.py` — `TrajectoryVerifierEvaluator` class
  ...
  - Formats trajectory: `[USER] prompt`, `[TOOL CALL] name(args)`, `[TOOL RESULT] output`, `[ASSISTANT] response`
  - Truncates to `max_trajectory_tokens` (head-and-tail strategy)
  - Structured prompt asks for 5 JSON scores (0.0-1.0):
    1. `trajectory_solution_quality` — Does the final output correctly solve the task?
    2. `trajectory_approach_efficiency` — Did the agent take a reasonable path? (no loops, no wasted steps)
    3. `trajectory_code_quality` — Is the generated code clean, idiomatic, well-structured?
...
- [x] (2026-03-02) `workers/tests/test_trajectory_verifier.py` — Tests:
  ...
  - LLM response parse failure → error dimension
...
#### 28C: Multi-Rollout Test-Time Scaling ✅ (2026-03-02)

> **Insight:** R2E-Gym/EntroPO demonstrate that running N independent rollouts and selecting the best
> via hybrid verification dramatically improves results (59.8% @16 rollouts vs 51.6% @1).
> CodeForge currently runs each benchmark task exactly once. This adds configurable N-rollout execution
...
- [x] (2026-03-02) `workers/codeforge/evaluation/runners/multi_rollout.py` — `MultiRolloutRunner` class
  ...
  - `run_task(task) -> list[RolloutOutcome]`:
    1. Run `inner_runner.run_task(task)` N times independently
    2. Compute diversity scores: normalized edit distance between `actual_output` of each pair, averaged per rollout
    3. If `strategy == "best"` and `hybrid_pipeline`: call `verify_batch()` → sort by combined score → mark best
...
- [x] (2026-03-02) `internal/adapter/postgres/migrations/055_multi_rollout.sql`:
  ...
  ALTER TABLE benchmark_results
      ADD COLUMN rollout_id       INTEGER NOT NULL DEFAULT 0,
      ADD COLUMN rollout_count    INTEGER NOT NULL DEFAULT 1,
      ADD COLUMN is_best_rollout  BOOLEAN NOT NULL DEFAULT TRUE,
      ADD COLUMN diversity_score  DOUBLE PRECISION NOT NULL DEFAULT 0.0;
  CREATE INDEX idx_bench_results_rollout ON benchmark_results(run_id, task_id, rollout_id);
  ALTER TABLE benchmark_runs
      ADD COLUMN rollout_count    INTEGER NOT NULL DEFAULT 1,
      ADD COLUMN rollout_strategy TEXT NOT NULL DEFAULT 'best';
  -- +goose Down
...
#### 28D: Diversity-Aware MAB Routing ✅ (2026-03-02)

> **Insight:** EntroPO proves that diversity matters — DPO/standard alignment causes "diversity collapse"
> where models converge to the same narrow solution set. Standard UCB1 in CodeForge's MAB similarly
> converges to always picking the same "best" model. When test-time scaling (multi-rollout) is active,
> picking N diverse models outperforms picking the same model N times.
>
> Key formula: `entropy_ucb1 = avg_reward + c * sqrt(ln(N)/n_i) + lambda * (-log(p_i))`
...
- [x] (2026-03-02) `workers/codeforge/routing/mab.py` — Add two methods to `MABModelSelector`:
  - `_entropy_ucb1_score(stats, total_trials, selection_counts) -> float`:
    ...
    - Return `base_ucb1 + entropy_weight * entropy_bonus`
  - `select_diverse(task_type, complexity_tier, available_models, n=1, max_cost=None) -> list[str]`:
...
- [x] (2026-03-02) `workers/tests/test_routing_entropy.py` — Tests:
  ...
  - `select_diverse(n=5)` with 2 available models → reuses models (graceful)
  - `entropy_weight=0.0` → collapses back to standard UCB1
  - Extreme `entropy_weight` (e.g. 10.0) → always picks underexplored models first
  - Empty available models → empty list returned
  - Cold start (all under min_trials) → returns None for each selection

**Reuses:** `MABModelSelector._ucb1_score()` (composed, not replaced), `ModelStats`, `RoutingConfig`, `ComplexityTier`/`TaskType` enums

---

#### 28E: Trajectory Export for DPO / EntroPO Training ✅ (2026-03-02)

> **Insight:** R2E-Gym and EntroPO train models using chosen/rejected trajectory pairs (DPO format).
> When multi-rollout is active, the best rollout becomes "chosen" and worse rollouts become "rejected".
> CodeForge can export these pairs in JSONL format (HuggingFace-compatible) for external model fine-tuning.
...
- [x] (2026-03-02) `workers/codeforge/evaluation/export/trajectory_exporter.py` — `TrajectoryExporter` class + Pydantic models:
  ...
  - `TrainingDataExport(version, source, pairs, total_tasks, total_pairs, model, benchmark_suite)` — full export package
  - `TrajectoryExporter.export_pairs(results: list[BenchmarkTaskResult]) -> list[TrainingPair]`:
    - Group results by `task_id`
    - For tasks with `rollout_count >= 2`: best rollout = chosen, each other = rejected → N-1 pairs per task
...
- [x] (2026-03-02) `workers/tests/test_trajectory_exporter.py` — Tests:
  ...
  - JSONL output format validity (each line is valid JSON)
  - `score_gap` computation correctness
...
#### 28F: SWE-GEN Synthetic Task Generation ✅ (2026-03-02)

> **Insight:** R2E-Gym's SWE-GEN pipeline auto-generates executable benchmark environments from Git commits:
> extract diff → LLM generates test for the change → LLM back-translates diff into natural language issue.
> This turns any CodeForge-managed project's Git history into a custom benchmark suite, enabling
> project-specific agent evaluation without manually writing test cases.
>
...
- [x] (2026-03-02) `workers/codeforge/evaluation/generators/swegen.py` — `SWEGenPipeline` class:
  ...
  - `generate_from_commits(commits: list[CommitInfo]) -> list[SyntheticTask]`:
    - For each commit: run `_generate_test()` + `_generate_issue()` concurrently (asyncio.gather)
    - Filter: skip if test generation fails, skip if issue is too generic
  - `_generate_test(commit) -> str` — LLM prompt: "Given this diff, write a test that FAILS before and PASSES after"
...
- [x] (2026-03-02) `workers/codeforge/evaluation/providers/codeforge_synthetic.py` — Benchmark provider:
  ...
  - Constructor: `workspace_path`, `llm`, `max_tasks: int = 50`, `model`
  - `load_tasks()` — runs SWEGenPipeline on project's git history, caches result
  - `_to_task_spec(SyntheticTask) -> TaskSpec`:
    - `id = f"synthetic-{parent_sha[:8]}"`, `input = issue_description`
...
- [x] (2026-03-02) `workers/tests/test_synthetic_provider.py` — Tests:
  ...
  - Test command inference (Python→`pytest`, Go→`go test ./...`, Node→`npm test`)
  - TaskSpec construction from SyntheticTask
  - Mock LLM responses for issue + test generation
...
**Reuses:** `BenchmarkProvider` protocol, `register_provider()`, `TaskSpec`/`Capabilities` types, `BenchmarkType.AGENT`, LiteLLM client, Git integration (via `subprocess` git log/diff commands)
...
```
28A (Hybrid Pipeline) — no deps, foundation for everything
  ↓
28B (Trajectory Verifier) — depends on 28A (is a Stage 2 evaluator)
  ↓
28C (Multi-Rollout) ←→ 28D (Diversity MAB) — can be parallel, 28C uses 28A+28B
  ↓
28E (Trajectory Export) — depends on 28C (needs multi-rollout data)
  |
28F (Synthetic Tasks) — fully independent, parallel with anything
  ↓
28G (Integration & Docs) — after all areas complete
```
...
### Phase 30: Goal Discovery — Project-Aware Context for Agents (2026-03-02)

> **Purpose:** Agents currently receive only code and conversation as context. Goal Discovery auto-detects
> project vision, requirements, constraints, and state from workspace files (`.planning/`, `CLAUDE.md`,
> `README.md`, etc.) and injects them into the agent's context window. This gives agents a "north star"
> so they make decisions aligned with the project's actual goals, not just the immediate task prompt.
>
> **Architecture:** Three-tier file detection (GSD `.planning/` → agent instructions → project docs)
> → domain model `goal.ProjectGoal` → PostgreSQL persistence → context injection as `EntryGoal` entries.
>
> **Audit status (2026-03-02):** Domain model, DB layer, migration, service logic, and detection are
> implemented with tests. However, the feature is **disconnected from all production entry points**.
> The audit below tracks exactly what remains.

#### 30A: Domain Model & Database Layer ✅ (2026-03-02)

- [x] (2026-03-02) Domain model: `internal/domain/goal/goal.go` — `GoalKind` enum (vision, requirement, constraint, state, context), `ProjectGoal` struct, `CreateRequest`, `UpdateRequest`, validation methods
- [x] (2026-03-02) Domain tests: `internal/domain/goal/goal_test.go` — 4 table-driven tests (GoalKind validation, CreateRequest validation with 8 cases, UpdateRequest validation, ProjectGoal validation)
- [x] (2026-03-02) Migration: `internal/adapter/postgres/migrations/056_project_goals.sql` — `project_goals` table with tenant isolation, kind CHECK constraint, 2 indexes (project, enabled)
- [x] (2026-03-02) Postgres store: `internal/adapter/postgres/store_project_goal.go` — full CRUD (Create, Get, List, ListEnabled, Update, Delete, DeleteBySource) with `scanGoals` helper
- [x] (2026-03-02) Store interface: `internal/port/database/store.go` — 7 new methods on `Store` interface (CreateProjectGoal, GetProjectGoal, ListProjectGoals, ListEnabledGoals, UpdateProjectGoal, DeleteProjectGoal, DeleteProjectGoalsBySource)
- [x] (2026-03-02) Context kind: `internal/domain/context/pack.go` — added `EntryGoal` to `EntryKind` enum and `ValidEntryKind` switch

#### 30B: Service Layer & Detection Logic ✅ (2026-03-02)

- [x] (2026-03-02) Service: `internal/service/goal_discovery.go` — `GoalDiscoveryService` with DetectAndImport, Create, Get, List, ListEnabled, Update, Delete, AsContextEntries
- [x] (2026-03-02) Detection: Three-tier workspace scanning (GSD `.planning/`, agent instructions, project docs)
...
- [x] (2026-03-07) **Go:** Add `Agentic bool` field to `ConversationRunStartPayload` in `internal/port/messagequeue/schemas.go` (~line 428)
  ```go
  Agentic bool `json:"agentic"` // false = simple single-turn chat, true = multi-turn tool loop
  ```
- [x] (2026-03-07) **Python:** Add `agentic: bool = True` field to corresponding Pydantic model in `workers/codeforge/models.py` (~line 415)
- [x] (2026-03-07) **Tests:** Verify JSON round-trip (Go marshal -> Python unmarshal) for both `agentic: true` and `agentic: false`

##### Step A2: Python Worker — Simple chat handler

- [x] (2026-03-07) **File:** `workers/codeforge/consumer/_conversation.py` — in `_handle_conversation_run()`
- [x] (2026-03-07) Add branch after existing setup (runtime, tools, history, routing):
  ```python
  ...
  ```
- [x] (2026-03-07) Implement `_run_simple_chat()` method:
...
- [x] (2026-03-07) **Tests:** Unit test for `_run_simple_chat()` with mocked LLM client and runtime

##### Step A3: Per-chunk streaming in Python Worker

- [x] (2026-03-07) **File:** `workers/codeforge/agent_loop.py` (~line 300)
- [x] (2026-03-07) Current behavior: `on_chunk=streamed_text.append` -> sends full text as one blob at end
- [x] (2026-03-07) For simple mode (non-agentic): pass `on_chunk` that calls `runtime.send_output()` per chunk
- [x] (2026-03-07) For agentic mode: keep current behavior (full text after each iteration) — agent loop needs complete text to decide on tool calls
- [x] (2026-03-07) **File:** `workers/codeforge/llm.py` (lines 600-659) — verify `chat_completion_stream()` SSE parsing delivers chunks correctly to callback
- [x] (2026-03-07) **Tests:** Verify per-chunk streaming sends multiple `runs.output` NATS messages (not one blob)

##### Step A4: Go Core — Replace direct LLM call with NATS dispatch

- [x] (2026-03-07) **File:** `internal/service/conversation.go` (lines 193-328) — `SendMessage()` method
- [x] (2026-03-07) Replace direct LiteLLM streaming path with NATS dispatch:
...
- [x] (2026-03-07) Response storage and `AGUIRunFinished` broadcast already handled by `HandleConversationRunComplete()` (existing NATS completion handler)
- [x] (2026-03-07) **Reuse from `SendMessageAgentic()`:**
...
- [x] (2026-03-07) **Tests:** Go unit test verifying `SendMessage()` publishes to NATS instead of calling LiteLLM directly

##### Step A5: HTTP handler — Unified async response

- [x] (2026-03-07) **File:** `internal/adapter/http/handlers_conversation.go` (~lines 79-107)
- [x] (2026-03-07) Currently: agentic returns `202 Accepted` with `{ run_id }`, simple returns `201 Created` with full message (blocks until LLM done)
- [x] (2026-03-07) Change to: **Always return `202 Accepted`** with `{ run_id }` — frontend already handles this for agentic mode
- [x] (2026-03-07) Remove the branching logic between `SendMessage()` (201) and `SendMessageAgentic()` (202) — both now async
- [x] (2026-03-07) **Tests:** HTTP handler test verifying 202 response for all message types

##### Step A6: Cleanup — Remove direct LiteLLM dependency from conversation service

- [x] (2026-03-07) **File:** `internal/service/conversation.go` — after migration, `SendMessage()` no longer needs `s.llm.ChatCompletionStream()`
- [x] (2026-03-07) Remove `llm` field from `ConversationService` if no other method uses it for chat (keep `resolveModel()` which reads config, not LLM)
- [x] (2026-03-07) Update `cmd/codeforge/main.go` wiring if ConversationService constructor changes
- [x] (2026-03-07) **Tests:** Verify Go compilation, run `go test ./internal/service/... -v -run Conversation`

#### Part B: Global Run Tracking (Frontend)

##### Step B1: Create `ConversationRunProvider`

- [x] (2026-03-07) **New file:** `frontend/src/components/ConversationRunProvider.tsx`
- [x] (2026-03-07) Follow established provider pattern (`SidebarProvider`, `AuthProvider`, `WebSocketProvider`):
  ```typescript
  ...
  ```
- [x] (2026-03-07) Subscribe to `agui.run_started` -> add `run_id` to set
- [x] (2026-03-07) Subscribe to `agui.run_finished` -> remove `run_id` from set
- [x] (2026-03-07) Lives at app root (between `WebSocketProvider` and route content)
- [x] (2026-03-07) Survives page navigation (unlike ChatPanel-local state)

##### Step B2: Wire into App component tree

- [x] (2026-03-07) **File:** `frontend/src/App.tsx`
- [x] (2026-03-07) Add `<ConversationRunProvider>` after `<WebSocketProvider>`:
  ```tsx
  <WebSocketProvider>
...
- [x] (2026-03-07) **File:** `frontend/src/components/Sidebar.tsx` (or equivalent navigation component)
- [x] (2026-03-07) Use `useConversationRuns()` hook from the provider
- [x] (2026-03-07) Show a pulsing dot / badge next to "Chat" nav item when `activeRuns().size > 0`
- [x] (2026-03-07) Minimal UI change — just an indicator, not a full panel

##### Step B4: ChatPanel consults global tracker on mount

- [x] (2026-03-07) **File:** `frontend/src/features/project/ChatPanel.tsx`
- [x] (2026-03-07) On mount:
...
- [x] (2026-03-07) Seamless resume: user navigates away, comes back, sees run still active with live streaming

##### Step B5: Unified HTTP response handling in ChatPanel

- [x] (2026-03-07) **File:** `frontend/src/features/project/ChatPanel.tsx`
- [x] (2026-03-07) Currently `sendMessage()` handles two response types (201 with message vs 202 with run_id)
- [x] (2026-03-07) After Part A, it's always 202 — simplify the handler to only handle `{ run_id }` response
- [x] (2026-03-07) Remove dead code for 201 response parsing
...
- [x] (2026-03-07) **Unit tests (Python):** `cd workers && python -m pytest tests/ -v` — all pass including new simple chat tests
- [x] (2026-03-07) **Unit tests (Go):** `go test ./internal/service/... -v -run Conversation` — SendMessage uses NATS
- [x] (2026-03-07) **Go compilation:** `go build ./...` — no unused imports or dead code
- [x] (2026-03-07) **Pre-commit:** `pre-commit run --all-files` — clean
- [x] (2026-03-07) **E2E (manual):**
...
- [x] (2026-03-07) **Regression:** `cd workers && python -m pytest tests/ -v` and `go test ./... -count=1`
