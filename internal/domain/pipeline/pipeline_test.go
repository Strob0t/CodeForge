...
func TestInstantiate_EmptyBinding_AutoFills(t *testing.T) {
	tmpl := Template{
		ID:       "test",
		Name:     "Test",
		Protocol: plan.ProtocolSequential,
	...
	if result.Steps[0].TaskID == "" {
		t.Error("empty TaskID should be auto-filled with UUID")
	}
	if result.Steps[0].AgentID != "a" {
		t.Errorf("AgentID = %q, want %q (should be preserved)", result.Steps[0].AgentID, "a")
	}
}

func TestInstantiate_NilBindings_AutoGenerated(t *testing.T) {
	tmpl := Template{
		ID:       "test",
		Name:     "Test",
		Protocol: plan.ProtocolSequential,
		Steps: []Step{
			...
			{Name: "Code", ModeID: "coder", DependsOn: []int{0}},
		},
	}
	...
	for i, s := range result.Steps {
		if s.TaskID == "" {
			t.Errorf("step %d: TaskID should be auto-generated", i)
		}
		if s.AgentID == "" {
			t.Errorf("step %d: AgentID should be auto-generated", i)
		}
	}
	// Auto-generated UUIDs should be unique.
	if result.Steps[0].TaskID == result.Steps[1].TaskID {
		t.Error("auto-generated TaskIDs should be unique per step")
	}
}

func TestInstantiate_PartialBindings_AutoFill(t *testing.T) {
	tmpl := Template{
		ID:       "test",
		Name:     "Test",
		Protocol: plan.ProtocolSequential,
	...
	result, err := tmpl.Instantiate(InstantiateRequest{
		ProjectID: "p",
		Bindings:  []StepBinding{{TaskID: "explicit-task", AgentID: ""}},
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.Steps[0].TaskID != "explicit-task" {
		t.Errorf("TaskID = %q, want %q (should be preserved)", result.Steps[0].TaskID, "explicit-task")
	}
	if result.Steps[0].AgentID == "" {
		t.Error("empty AgentID should be auto-filled with UUID")
	}
}
