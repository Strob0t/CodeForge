...
// Register adds a PM provider factory under the given name.
...
func Register(name string, factory Factory) {
	...
	if _, exists := factories[name]; exists {
		panic(fmt.Sprintf("pmprovider: duplicate registration %q", name))
	}
...
func New(name string, config map[string]string) (Provider, error) {
	...
	if !ok {
		return nil, fmt.Errorf("pmprovider: unknown provider %q", name)
	}
