...
func mountRoadmapRoutes(r chi.Router, h *Handlers) {
	...
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Post("/projects/{id}/roadmap", h.CreateProjectRoadmap)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Put("/projects/{id}/roadmap", h.UpdateProjectRoadmap)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Delete("/projects/{id}/roadmap", h.DeleteProjectRoadmap)
	r.Get("/projects/{id}/roadmap/ai", h.GetRoadmapAI)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Post("/projects/{id}/roadmap/detect", h.DetectRoadmap)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Post("/projects/{id}/roadmap/import", h.ImportSpecs)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Post("/projects/{id}/roadmap/import/pm", h.ImportPMItems)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Post("/projects/{id}/roadmap/milestones", h.CreateMilestone)

	// Milestones (direct access)
	r.Get("/milestones/{id}", h.GetMilestone)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Put("/milestones/{id}", h.UpdateMilestone)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Delete("/milestones/{id}", h.DeleteMilestone)

	// Features (nested under milestones + direct access)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Post("/milestones/{id}/features", h.CreateFeature)
	r.Get("/features/{id}", h.GetFeature)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Put("/features/{id}", h.UpdateFeature)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Delete("/features/{id}", h.DeleteFeature)

	// Bidirectional Sync (nested under projects)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Post("/projects/{id}/roadmap/sync", h.SyncRoadmap)
	r.With(middleware.RequireRole(user.RoleAdmin, user.RoleEditor)).Post("/projects/{id}/roadmap/sync-to-file", h.SyncToSpecFile)
}
