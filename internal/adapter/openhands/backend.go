// Package openhands implements the agentbackend.Backend interface for the OpenHands coding agent.
package openhands
...
const backendName = "openhands"

// Backend dispatches tasks to a Python worker running OpenHands via NATS.
type Backend struct {
	queue messagequeue.Queue
}

// New creates an OpenHands backend with the given NATS queue.
func New(queue messagequeue.Queue) *Backend {
	return &Backend{queue: queue}
}

// Register registers the OpenHands backend factory with the given NATS queue.
...
// Capabilities returns what OpenHands supports.
...
// Execute dispatches a task to the Python OpenHands worker via NATS.
...
func (b *Backend) Execute(ctx context.Context, t *task.Task) (*task.Result, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("openhands: marshal task: %w", err)
	}

	subject := messagequeue.SubjectTaskAgent + "." + backendName
	if err := b.queue.Publish(ctx, subject, data); err != nil {
		return nil, fmt.Errorf("openhands: publish task: %w", err)
	}
...
func (b *Backend) Stop(ctx context.Context, taskID string) error {
	...
	if err != nil {
		return fmt.Errorf("openhands: marshal cancel: %w", err)
	}

	if err := b.queue.Publish(ctx, messagequeue.SubjectTaskCancel, data); err != nil {
		return fmt.Errorf("openhands: publish cancel: %w", err)
	}
